...
def main(page: ft.Page):
    ...
    def on_update_frame_index(value):
        sldr_time_bar.value = value
        ft_time_line_value.value = f'{video_viewer.get_current_time()}/{video_viewer.get_total_time()}'
        page.update()
    ...
    #             if d_time.microseconds/1000000 < latency and d_time.microseconds > 0.0:
    #                sleep(latency - d_time.microseconds/1000000) # Здесь необходимо замерять время между вызовами, и если и ожидать только разницу.
    #         else:
    #             break
    #     if capture:
    #         capture.release()
    ...
    video_viewer = VideoViewer(update_frame_index=on_update_frame_index)
    tf_page_size = ft.TextField(value='', expand=True)

    ft_row = ft.Column([video_viewer, ft.Row([tf_page_size]), ft_play_track_coontainer],
                        # width = page.width-10
                        expand=True
                        )
